...
export default function BrandList({ brands, flash }) {
    ...
    const handleSearch = (e) => {
        e.preventDefault();
        router.get(route('brand.index'), { search: searchQuery, limit }, { preserveState: true, preserveScroll: true });
    };

    const handleLimitChange = (e) => {
        setLimit(e.target.value);
        router.get(route('brand.index'), { search: searchQuery, limit: e.target.value }, { preserveState: true, preserveScroll: true });
    };
    ...
    const openEditModal = (brand) => {
        setForm({
            ...
            status: brand.status,
            id: brand.id,
            image: null, // file input harus null
        });
        setOldImage(brand.image || null); // simpan path lama untuk preview
        setModalType('edit');
        setErrors({});
        setShowModal(true);
    };
    ...
    const handleSubmit = (e) => {
        ...
        if (modalType === 'add') {
            router.post(route('brand.store'), data, {
                onSuccess: () => {
                    setShowModal(false);
                    setOldImage(null);
                },
            ...
            router.post(route('brand.update', form.id), data, {
                onSuccess: () => {
                    setShowModal(false);
                    setOldImage(null);
                },
    ...
    const confirmDelete = () => {
        router.put(route('brand.delete', deleteId), { status: 2 }, {
            preserveScroll: true,
            onSuccess: () => {
                        ...
                        <div className="lg:p-6 sm:p-2 text-gray-900">
                                ...
                                <table className="min-w-full divide-y divide-gray-200 text-sm">
                                    ...
                                    <tbody className="bg-white divide-y divide-gray-200">
                                        {brands.data.length === 0 && (
                                            <tr>
                                                <td colSpan={4} className="text-center py-4 text-gray-500">Tidak ada data brand</td>
                                            </tr>
                                        )}
                                        {brands.data.map((brand, idx) => (
                                            <tr key={brand.id} className="hover:bg-gray-50">
                                                <td className="px-2 py-2 whitespace-nowrap">{number++}</td>
                                                <td className="px-2 py-2 whitespace-nowrap">{brand.name}</td>
                                                <td className="px-2 py-2 whitespace-nowrap">
                                                    {brand.image ? (
                                                        <button
                                                            type="button"
                                                            onClick={() => {
                                                                setImageUrl(`/storage/${brand.image}`);
                            ...
                            <div className="px-6 py-4 flex flex-col gap-2 sm:flex-row sm:justify-between sm:items-center">
                                <div className="text-center sm:text-left text-gray-500 ">
                                    Menampilkan {brands.from} sampai {brands.to} dari {brands.total} brand
                                </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
                    <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md">
                        <h3 className="text-lg font-semibold mb-4">
                            {modalType === 'add' ? 'Tambah Brand' : 'Edit Brand'}
                        </h3>
